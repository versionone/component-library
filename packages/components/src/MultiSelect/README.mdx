        ...
        const renderOptions = ({ getItemProps }) => {
          const group = Object.keys(this.state.items).map((id, index) => {
            ...
            return (
              ...
              >
                <ListItemText primary={item.value} secondary={item.email} />
              </ListItem>
            );
          });
          return (
...
<Playground>
  <SpacedGroup direction="vertical">
    <FormLabel center label="My Label">
...
Provide `getItemProps` with the `id` and the `index` of each item and it will return props that can be appliedto your ListItem UI.
