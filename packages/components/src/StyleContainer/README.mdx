...
This package the primary way to create custom, theme-enabled, styled components; along with several other utilities relating to styling components. The main items you will likely use from this package are:
  ...
  {() => {
    ...
    return (
      <Fragment>
        ...
        <Heading color="lightblue" onClick={console.log}>This header does not pass through onClick prop</Heading>
        <ClickableHeading color="lightblue" onClick={console.log}>This header **does pass through** onClick prop</ClickableHeading>
      </Fragment>
    );
...
<Playground>
  {() => {
    ...
    return (
      <Fragment>
        <Heading color="lightblue">This has a custom padding and no margins</Heading>
        <Heading hasMargins color="lightblue">This has a custom padding and has margins</Heading>
      </Fragment>
    );
...
```js
import { WithRenderer } from '@versionone/compnents/StyleContainer';
...
> Be aware, that global styles are global and once added to the page, are not removed.

<Playground>
  {() => {
    // Try changing 'white' to 'blue' and then back to 'white'
    const globalStyles = {
      backgroundColor: 'white',
    };

    const ThemedHeading = applyStaticStyles(globalStyles, 'p')(createComponent(() => ({}), 'h3'));

    return <ThemedHeading>I have global styles</ThemedHeading>
